...
def plot_star_model(space_name, scores, save_path):
    ...
    # 固定五条维度轴线角度，保证 B1 在正上方，顺时针依次为 B2/B3/B4/B5
    angles_outer = np.deg2rad([90, 18, -54, -126, 162])
    angles_inner = np.deg2rad([54, -18, -90, -162, 126])
    axis_labels = [
        "使用对象多元性（B1）",
        "管理控制性（B2）",
        "公共服务与维护性（B3）",
    ...
    # 计算外凸点与内凹点坐标
    outer_points = [polar_to_xy(outer_r, angle) for angle in angles_outer]
    inner_points = [polar_to_xy(inner_r, angle) for angle in angles_inner]

    # 绘制完整五角星边框：外凸点 -> 内凹点 -> 外凸点 -> ... -> 回到首个外凸点
    star_points = []
    for i in range(5):
    ...
    ax.plot(star_x + [star_x[0]], star_y + [star_y[0]], color="black", lw=1.3, zorder=4)

    # 将凹点与中心点用黑色线条连接，形成星形辅助结构
    for point in inner_points:
        ax.plot([0, point[0]], [0, point[1]], color="black", lw=0.9, alpha=0.9, zorder=2)
    ...
    for i, (code, _, _) in enumerate(DIMENSION_ORDER):
        ...
        if pd.notna(score):
            radius = float(score)
            radius = max(0.0, min(5.0, radius))
        else:
            radius = 0.0
        data_radii.append(radius)
        x, y = polar_to_xy(radius, angles_outer[i])
        data_points.append((x, y))

    # 为每个维度绘制独立四边形色块：中心点 -> 左凹点 -> 当前维度数值点 -> 右凹点 -> 回到中心点
    for i in range(5):
        center = (0.0, 0.0)
        left_concave = inner_points[(i - 1) % 5]
        value_point = data_points[i]
        right_concave = inner_points[i]
